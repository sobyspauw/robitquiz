// Authors and Writers Quiz - Level 6: Knowledge about Authors and Writers
(function() {
  const level6 = {
    name: {
      en: "Authors and Writers Level 6",
      es: "Autores y Escritores Nivel 6",
      de: "Autoren und Schriftsteller Stufe 6",
      nl: "Auteurs en Schrijvers Level 6"
    },
    questions: [
      {
        question: {
          en: "Which modernist writer's stream-of-consciousness technique influenced James Joyce's 'Ulysses'?",
          es: "¿La técnica de flujo de conciencia de qué escritor modernista influyó en 'Ulises' de James Joyce?",
          de: "Welches modernistischen Schriftstellers Bewusstseinsstrom-Technik beeinflusste James Joyces 'Ulysses'?",
          nl: "Welke modernistische schrijver's bewustzijnsstroom-techniek beïnvloedde James Joyce's 'Ulysses'?"
        },
        options: [
          {en: "Édouard Dujardin", es: "Édouard Dujardin", de: "Édouard Dujardin", nl: "Édouard Dujardin"},
          {en: "Marcel Proust", es: "Marcel Proust", de: "Marcel Proust", nl: "Marcel Proust"},
          {en: "Dorothy Richardson", es: "Dorothy Richardson", de: "Dorothy Richardson", nl: "Dorothy Richardson"},
          {en: "Henry James", es: "Henry James", de: "Henry James", nl: "Henry James"}
        ],
        correct: 0,
        explanation: {
          en: "Édouard Dujardin's 1888 novel 'Les Lauriers sont coupés' pioneered the stream-of-consciousness technique. Joyce himself acknowledged Dujardin as the inventor of interior monologue, which became a cornerstone of modernist literature.",
          es: "La novela de 1888 'Les Lauriers sont coupés' de Édouard Dujardin fue pionera en la técnica del flujo de conciencia. El propio Joyce reconoció a Dujardin como el inventor del monólogo interior, que se convirtió en una piedra angular de la literatura modernista.",
          de: "Édouard Dujardins Roman 'Les Lauriers sont coupés' von 1888 war wegweisend für die Bewusstseinsstrom-Technik. Joyce selbst erkannte Dujardin als den Erfinder des inneren Monologs an, der zu einem Grundpfeiler der modernistischen Literatur wurde.",
          nl: "Édouard Dujardin's roman 'Les Lauriers sont coupés' uit 1888 was baanbrekend voor de bewustzijnsstroom-techniek. Joyce zelf erkende Dujardin als de uitvinder van de innerlijke monoloog, die een hoeksteen werd van de modernistische literatuur."
        }
      },
      {
        question: {
          en: "Which American Beat poet wrote 'Howl', a poem that led to an obscenity trial in 1957?",
          es: "¿Qué poeta Beat estadounidense escribió 'Aullido', un poema que llevó a un juicio por obscenidad en 1957?",
          de: "Welcher amerikanische Beat-Dichter schrieb 'Howl', ein Gedicht, das 1957 zu einem Obszönitätsprozess führte?",
          nl: "Welke Amerikaanse Beat-dichter schreef 'Howl', een gedicht dat in 1957 leidde tot een obsceniteitsproces?"
        },
        options: [
          {en: "Jack Kerouac", es: "Jack Kerouac", de: "Jack Kerouac", nl: "Jack Kerouac"},
          {en: "Allen Ginsberg", es: "Allen Ginsberg", de: "Allen Ginsberg", nl: "Allen Ginsberg"},
          {en: "Gary Snyder", es: "Gary Snyder", de: "Gary Snyder", nl: "Gary Snyder"},
          {en: "Lawrence Ferlinghetti", es: "Lawrence Ferlinghetti", de: "Lawrence Ferlinghetti", nl: "Lawrence Ferlinghetti"}
        ],
        correct: 1,
        explanation: {
          en: "Allen Ginsberg wrote 'Howl' in 1955, and its publication by City Lights Bookstore led to the arrest of Lawrence Ferlinghetti on obscenity charges. The subsequent trial, which Ferlinghetti won, was landmark for free speech in literature.",
          es: "Allen Ginsberg escribió 'Aullido' en 1955, y su publicación por City Lights Bookstore llevó al arresto de Lawrence Ferlinghetti por cargos de obscenidad. El juicio subsecuente, que Ferlinghetti ganó, fue histórico para la libertad de expresión en literatura.",
          de: "Allen Ginsberg schrieb 'Howl' 1955, und seine Veröffentlichung durch City Lights Bookstore führte zur Verhaftung von Lawrence Ferlinghetti wegen Obszönität. Der darauffolgende Prozess, den Ferlinghetti gewann, war wegweisend für die Meinungsfreiheit in der Literatur.",
          nl: "Allen Ginsberg schreef 'Howl' in 1955, en de publicatie door City Lights Bookstore leidde tot de arrestatie van Lawrence Ferlinghetti op beschuldiging van obsceniteit. Het daaropvolgende proces, dat Ferlinghetti won, was baanbrekend voor vrije meningsuiting in literatuur."
        }
      },
      {
        question: {
          en: "Which Russian author's novel 'The Master and Margarita' was published posthumously after being banned for decades?",
          es: "¿La novela 'El maestro y Margarita' de qué autor ruso se publicó póstumamente después de estar prohibida durante décadas?",
          de: "Welches russischen Autors Roman 'Der Meister und Margarita' wurde posthum veröffentlicht, nachdem er jahrzehntelang verboten war?",
          nl: "Welke Russische auteur's roman 'De Meester en Margarita' werd postuum gepubliceerd nadat het decennia lang verboden was?"
        },
        options: [
          {en: "Mikhail Bulgakov", es: "Mikhail Bulgakov", de: "Mikhail Bulgakov", nl: "Mikhail Bulgakov"},
          {en: "Vasily Grossman", es: "Vasily Grossman", de: "Vasily Grossman", nl: "Vasily Grossman"},
          {en: "Andrei Platonov", es: "Andrei Platonov", de: "Andrei Platonov", nl: "Andrei Platonov"},
          {en: "Varlam Shalamov", es: "Varlam Shalamov", de: "Varlam Shalamov", nl: "Varlam Shalamov"}
        ],
        correct: 0,
        explanation: {
          en: "Mikhail Bulgakov's 'The Master and Margarita' was completed in 1940 but only published in 1967, 27 years after the author's death. The novel's satirical portrayal of Soviet society and religious themes made it unpublishable during Stalin's era.",
          es: "'El maestro y Margarita' de Mikhail Bulgakov se completó en 1940 pero solo se publicó en 1967, 27 años después de la muerte del autor. El retrato satírico de la sociedad soviética y los temas religiosos de la novela la hicieron impublicable durante la era de Stalin.",
          de: "Mikhail Bulgakovs 'Der Meister und Margarita' wurde 1940 vollendet, aber erst 1967 veröffentlicht, 27 Jahre nach dem Tod des Autors. Die satirische Darstellung der sowjetischen Gesellschaft und religiöse Themen machten den Roman während Stalins Ära unveröffentlichbar.",
          nl: "Mikhail Bulgakov's 'De Meester en Margarita' werd voltooid in 1940 maar pas gepubliceerd in 1967, 27 jaar na de dood van de auteur. De satirische weergave van de Sovjetsamenleving en religieuze thema's maakten de roman onuitgeefbaar tijdens Stalin's tijdperk."
        }
      },
      {
        question: {
          en: "Which Latin American author pioneered the literary technique known as 'magical realism'?",
          es: "¿Qué autor latinoamericano fue pionero en la técnica literaria conocida como 'realismo mágico'?",
          de: "Welcher lateinamerikanische Autor war Pionier der literarischen Technik, die als 'magischer Realismus' bekannt ist?",
          nl: "Welke Latijns-Amerikaanse auteur was pionier van de literaire techniek bekend als 'magisch realisme'?"
        },
        options: [
          {en: "Jorge Luis Borges", es: "Jorge Luis Borges", de: "Jorge Luis Borges", nl: "Jorge Luis Borges"},
          {en: "Gabriel García Márquez", es: "Gabriel García Márquez", de: "Gabriel García Márquez", nl: "Gabriel García Márquez"},
          {en: "Isabel Allende", es: "Isabel Allende", de: "Isabel Allende", nl: "Isabel Allende"},
          {en: "Alejo Carpentier", es: "Alejo Carpentier", de: "Alejo Carpentier", nl: "Alejo Carpentier"}
        ],
        correct: 3,
        explanation: {
          en: "Alejo Carpentier coined the term 'lo real maravilloso' (the marvelous real) in 1949 and is considered the originator of magical realism. His novel 'The Kingdom of This World' (1949) was one of the first to fully employ this technique.",
          es: "Alejo Carpentier acuñó el término 'lo real maravilloso' en 1949 y se considera el originador del realismo mágico. Su novela 'El reino de este mundo' (1949) fue una de las primeras en emplear completamente esta técnica.",
          de: "Alejo Carpentier prägte 1949 den Begriff 'lo real maravilloso' (das wunderbare Reale) und gilt als Begründer des magischen Realismus. Sein Roman 'Das Reich von dieser Welt' (1949) war einer der ersten, der diese Technik vollständig anwendete.",
          nl: "Alejo Carpentier muntte de term 'lo real maravilloso' (het wonderbaarlijke werkelijke) in 1949 en wordt beschouwd als de bedenker van magisch realisme. Zijn roman 'Het koninkrijk van deze wereld' (1949) was een van de eerste die deze techniek volledig gebruikte."
        }
      },
      {
        question: {
          en: "Which French symbolist poet wrote 'Un Coup de Dés Jamais N'Abolira le Hasard', experimenting with typography and layout?",
          es: "¿Qué poeta simbolista francés escribió 'Un Coup de Dés Jamais N'Abolira le Hasard', experimentando con tipografía y diseño?",
          de: "Welcher französische symbolistische Dichter schrieb 'Un Coup de Dés Jamais N'Abolira le Hasard' und experimentierte mit Typografie und Layout?",
          nl: "Welke Franse symbolistische dichter schreef 'Un Coup de Dés Jamais N'Abolira le Hasard', experimenterend met typografie en lay-out?"
        },
        options: [
          {en: "Paul Verlaine", es: "Paul Verlaine", de: "Paul Verlaine", nl: "Paul Verlaine"},
          {en: "Arthur Rimbaud", es: "Arthur Rimbaud", de: "Arthur Rimbaud", nl: "Arthur Rimbaud"},
          {en: "Stéphane Mallarmé", es: "Stéphane Mallarmé", de: "Stéphane Mallarmé", nl: "Stéphane Mallarmé"},
          {en: "Charles Baudelaire", es: "Charles Baudelaire", de: "Charles Baudelaire", nl: "Charles Baudelaire"}
        ],
        correct: 2,
        explanation: {
          en: "Stéphane Mallarmé's 'Un Coup de Dés Jamais N'Abolira le Hasard' (1897) was revolutionary for its use of white space, varied typography, and spatial arrangement of words on the page, predating concrete poetry by decades.",
          es: "'Un Coup de Dés Jamais N'Abolira le Hasard' (1897) de Stéphane Mallarmé fue revolucionario por su uso del espacio en blanco, tipografía variada y disposición espacial de las palabras en la página, anticipándose a la poesía concreta por décadas.",
          de: "Stéphane Mallarmés 'Un Coup de Dés Jamais N'Abolira le Hasard' (1897) war revolutionär durch die Verwendung von Leerraum, variabler Typografie und räumlicher Anordnung der Wörter auf der Seite, Jahrzehnte vor der konkreten Poesie.",
          nl: "Stéphane Mallarmé's 'Un Coup de Dés Jamais N'Abolira le Hasard' (1897) was revolutionair vanwege het gebruik van witruimte, gevarieerde typografie en ruimtelijke rangschikking van woorden op de pagina, decennia voor concrete poëzie."
        }
      },
      {
        question: {
          en: "Which German-Jewish author wrote 'The Metamorphosis', exploring themes of alienation and absurdity?",
          es: "¿Qué autor judío-alemán escribió 'La metamorfosis', explorando temas de alienación y absurdo?",
          de: "Welcher deutsch-jüdische Autor schrieb 'Die Verwandlung' und erforschte Themen der Entfremdung und Absurdität?",
          nl: "Welke Duits-Joodse auteur schreef 'De Metamorfose', waarin thema's van vervreemding en absurditeit werden verkend?"
        },
        options: [
          {en: "Franz Kafka", es: "Franz Kafka", de: "Franz Kafka", nl: "Franz Kafka"},
          {en: "Heinrich Heine", es: "Heinrich Heine", de: "Heinrich Heine", nl: "Heinrich Heine"},
          {en: "Stefan Zweig", es: "Stefan Zweig", de: "Stefan Zweig", nl: "Stefan Zweig"},
          {en: "Walter Benjamin", es: "Walter Benjamin", de: "Walter Benjamin", nl: "Walter Benjamin"}
        ],
        correct: 0,
        explanation: {
          en: "Franz Kafka wrote 'The Metamorphosis' (1915), the surreal story of Gregor Samsa's transformation into an insect. Kafka's work profoundly influenced existentialist philosophy and gave birth to the term 'Kafkaesque' to describe bureaucratic absurdity.",
          es: "Franz Kafka escribió 'La metamorfosis' (1915), la historia surrealista de la transformación de Gregor Samsa en un insecto. La obra de Kafka influyó profundamente en la filosofía existencialista y dio origen al término 'kafkiano' para describir el absurdo burocrático.",
          de: "Franz Kafka schrieb 'Die Verwandlung' (1915), die surreale Geschichte von Gregor Samsas Verwandlung in ein Insekt. Kafkas Werk beeinflusste die existentialistische Philosophie tiefgreifend und schuf den Begriff 'kafkaesk' für bürokratische Absurdität.",
          nl: "Franz Kafka schreef 'De Metamorfose' (1915), het surrealistische verhaal van Gregor Samsa's transformatie in een insect. Kafka's werk beïnvloedde de existentialistische filosofie diepgaand en bracht de term 'Kafkaesk' voort om bureaucratische absurditeit te beschrijven."
        }
      },
      {
        question: {
          en: "Which Victorian author's unfinished novel 'The Mystery of Edwin Drood' remains unsolved to this day?",
          es: "¿La novela inacabada 'El misterio de Edwin Drood' de qué autor victoriano permanece sin resolver hasta el día de hoy?",
          de: "Welches viktorianischen Autors unvollendeter Roman 'Das Geheimnis des Edwin Drood' bleibt bis heute ungelöst?",
          nl: "Welke Victoriaanse auteur's onvoltooide roman 'Het mysterie van Edwin Drood' blijft tot op de dag van vandaag onopgelost?"
        },
        options: [
          {en: "Charles Dickens", es: "Charles Dickens", de: "Charles Dickens", nl: "Charles Dickens"},
          {en: "Wilkie Collins", es: "Wilkie Collins", de: "Wilkie Collins", nl: "Wilkie Collins"},
          {en: "Anthony Trollope", es: "Anthony Trollope", de: "Anthony Trollope", nl: "Anthony Trollope"},
          {en: "George Meredith", es: "George Meredith", de: "George Meredith", nl: "George Meredith"}
        ],
        correct: 0,
        explanation: {
          en: "Charles Dickens died in 1870 while writing 'The Mystery of Edwin Drood', leaving only six of the planned twelve installments complete. The central mystery—whether Edwin Drood was murdered—has spawned countless theories and attempted completions.",
          es: "Charles Dickens murió en 1870 mientras escribía 'El misterio de Edwin Drood', dejando solo seis de las doce entregas planeadas completas. El misterio central—si Edwin Drood fue asesinado—ha generado innumerables teorías e intentos de completar la obra.",
          de: "Charles Dickens starb 1870 während er 'Das Geheimnis des Edwin Drood' schrieb und hinterließ nur sechs der geplanten zwölf Folgen vollständig. Das zentrale Mysterium—ob Edwin Drood ermordet wurde—hat unzählige Theorien und Vervollständigungsversuche hervorgebracht.",
          nl: "Charles Dickens stierf in 1870 terwijl hij 'Het mysterie van Edwin Drood' schreef, waarbij slechts zes van de geplande twaalf afleveringen voltooid waren. Het centrale mysterie—of Edwin Drood vermoord werd—heeft talloze theorieën en voltooiingspogingen voortgebracht."
        }
      },
      {
        question: {
          en: "Which American author's experimental novel 'Gravity's Rainbow' won the Pulitzer Prize but was deemed 'unreadable' by some critics?",
          es: "¿La novela experimental 'El arco iris de gravedad' de qué autor estadounidense ganó el Premio Pulitzer pero fue considerada 'ilegible' por algunos críticos?",
          de: "Welches amerikanischen Autors experimenteller Roman 'Die Enden der Parabel' gewann den Pulitzer-Preis, wurde aber von einigen Kritikern als 'unlesbar' bezeichnet?",
          nl: "Welke Amerikaanse auteur's experimentele roman 'Gravity's Rainbow' won de Pulitzer Prijs maar werd door sommige critici als 'onleesbaar' beschouwd?"
        },
        options: [
          {en: "Don DeLillo", es: "Don DeLillo", de: "Don DeLillo", nl: "Don DeLillo"},
          {en: "Thomas Pynchon", es: "Thomas Pynchon", de: "Thomas Pynchon", nl: "Thomas Pynchon"},
          {en: "David Foster Wallace", es: "David Foster Wallace", de: "David Foster Wallace", nl: "David Foster Wallace"},
          {en: "William Gaddis", es: "William Gaddis", de: "William Gaddis", nl: "William Gaddis"}
        ],
        correct: 1,
        explanation: {
          en: "Thomas Pynchon's 'Gravity's Rainbow' (1973) was awarded the Pulitzer Prize for Fiction, though the advisory board initially refused to give the award, calling the novel 'unreadable', 'turgid', 'overwritten' and 'obscene'.",
          es: "'El arco iris de gravedad' (1973) de Thomas Pynchon recibió el Premio Pulitzer de Ficción, aunque el consejo asesor inicialmente se negó a dar el premio, llamando a la novela 'ilegible', 'ampulosa', 'sobreescrita' y 'obscena'.",
          de: "Thomas Pynchons 'Die Enden der Parabel' (1973) erhielt den Pulitzer-Preis für Fiktion, obwohl das Beratungsgremium zunächst die Vergabe des Preises ablehnte und den Roman als 'unlesbar', 'schwülstig', 'überschrieben' und 'obszön' bezeichnete.",
          nl: "Thomas Pynchon's 'Gravity's Rainbow' (1973) kreeg de Pulitzer Prijs voor Fictie, hoewel de adviesraad aanvankelijk weigerde de prijs toe te kennen en de roman 'onleesbaar', 'gezwollen', 'overgeschreven' en 'obsceen' noemde."
        }
      },
      {
        question: {
          en: "Which Japanese author created the concept of 'mono no aware' in classical literature?",
          es: "¿Qué autor japonés creó el concepto de 'mono no aware' en la literatura clásica?",
          de: "Welcher japanische Autor schuf das Konzept von 'mono no aware' in der klassischen Literatur?",
          nl: "Welke Japanse auteur creëerde het concept van 'mono no aware' in de klassieke literatuur?"
        },
        options: [
          {en: "Murasaki Shikibu", es: "Murasaki Shikibu", de: "Murasaki Shikibu", nl: "Murasaki Shikibu"},
          {en: "Sei Shōnagon", es: "Sei Shōnagon", de: "Sei Shōnagon", nl: "Sei Shōnagon"},
          {en: "Matsuo Bashō", es: "Matsuo Bashō", de: "Matsuo Bashō", nl: "Matsuo Bashō"},
          {en: "Ki no Tsurayuki", es: "Ki no Tsurayuki", de: "Ki no Tsurayuki", nl: "Ki no Tsurayuki"}
        ],
        correct: 0,
        explanation: {
          en: "Murasaki Shikibu, author of 'The Tale of Genji' (circa 1000 CE), is credited with developing 'mono no aware'—the bittersweet awareness of the impermanence of things. This aesthetic concept became central to Japanese literature and culture.",
          es: "Murasaki Shikibu, autora de 'La historia de Genji' (circa 1000 d.C.), se le atribuye el desarrollo de 'mono no aware'—la conciencia agridulce de la impermanencia de las cosas. Este concepto estético se volvió central en la literatura y cultura japonesa.",
          de: "Murasaki Shikibu, Autorin von 'Die Geschichte vom Prinzen Genji' (um 1000 n. Chr.), wird die Entwicklung von 'mono no aware' zugeschrieben—das bittersüße Bewusstsein für die Vergänglichkeit der Dinge. Dieses ästhetische Konzept wurde zentral für die japanische Literatur und Kultur.",
          nl: "Murasaki Shikibu, auteur van 'Het verhaal van Genji' (circa 1000 n.Chr.), wordt gecrediteerd met de ontwikkeling van 'mono no aware'—het bitterzoete bewustzijn van de vergankelijkheid van dingen. Dit esthetische concept werd centraal in de Japanse literatuur en cultuur."
        }
      },
      {
        question: {
          en: "Which American author's autobiography 'I Know Why the Caged Bird Sings' became a seminal work in African American literature?",
          es: "¿La autobiografía 'Sé por qué canta el pájaro enjaulado' de qué autora estadounidense se convirtió en una obra seminal en la literatura afroamericana?",
          de: "Welche amerikanische Autorin schrieb die Autobiografie 'Ich weiß, warum der gefangene Vogel singt', die zu einem wegweisenden Werk der afroamerikanischen Literatur wurde?",
          nl: "Welke Amerikaanse auteur's autobiografie 'I Know Why the Caged Bird Sings' werd een baanbrekend werk in de Afro-Amerikaanse literatuur?"
        },
        options: [
          {en: "Toni Morrison", es: "Toni Morrison", de: "Toni Morrison", nl: "Toni Morrison"},
          {en: "Maya Angelou", es: "Maya Angelou", de: "Maya Angelou", nl: "Maya Angelou"},
          {en: "Zora Neale Hurston", es: "Zora Neale Hurston", de: "Zora Neale Hurston", nl: "Zora Neale Hurston"},
          {en: "Alice Walker", es: "Alice Walker", de: "Alice Walker", nl: "Alice Walker"}
        ],
        correct: 1,
        explanation: {
          en: "Maya Angelou's 'I Know Why the Caged Bird Sings' (1969) was groundbreaking for its frank discussion of racism, trauma, and resilience. It was the first non-fiction bestseller by an African American woman and sparked both acclaim and controversy.",
          es: "'Sé por qué canta el pájaro enjaulado' (1969) de Maya Angelou fue revolucionario por su discusión franca del racismo, trauma y resistencia. Fue el primer bestseller de no ficción de una mujer afroamericana y generó tanto elogios como controversia.",
          de: "Maya Angelous 'Ich weiß, warum der gefangene Vogel singt' (1969) war bahnbrechend für seine offene Diskussion von Rassismus, Trauma und Widerstandsfähigkeit. Es war der erste Sachbuch-Bestseller einer afroamerikanischen Frau und löste sowohl Lob als auch Kontroversen aus.",
          nl: "Maya Angelou's 'I Know Why the Caged Bird Sings' (1969) was baanbrekend vanwege de openhartige discussie over racisme, trauma en veerkracht. Het was de eerste non-fictie bestseller van een Afro-Amerikaanse vrouw en veroorzaakte zowel lof als controverse."
        }
      },
      {
        question: {
          en: "Which Romanian playwright's 'Rhinoceros' is considered a masterpiece of the Theatre of the Absurd?",
          es: "¿'Rinoceronte' de qué dramaturgo rumano se considera una obra maestra del Teatro del Absurdo?",
          de: "Welches rumänischen Dramatikers 'Rhinoceros' gilt als Meisterwerk des Theaters des Absurden?",
          nl: "Welke Roemeense toneelschrijver's 'Rhinoceros' wordt beschouwd als een meesterwerk van het Theater van het Absurde?"
        },
        options: [
          {en: "Mircea Eliade", es: "Mircea Eliade", de: "Mircea Eliade", nl: "Mircea Eliade"},
          {en: "Eugène Ionesco", es: "Eugène Ionesco", de: "Eugène Ionesco", nl: "Eugène Ionesco"},
          {en: "Emil Cioran", es: "Emil Cioran", de: "Emil Cioran", nl: "Emil Cioran"},
          {en: "Tristan Tzara", es: "Tristan Tzara", de: "Tristan Tzara", nl: "Tristan Tzara"}
        ],
        correct: 1,
        explanation: {
          en: "Eugène Ionesco's 'Rhinoceros' (1959) is an allegory about conformity and the rise of fascism, where citizens gradually transform into rhinoceroses. Ionesco was a key figure in the Theatre of the Absurd movement alongside Beckett and Pinter.",
          es: "'Rinoceronte' (1959) de Eugène Ionesco es una alegoría sobre la conformidad y el ascenso del fascismo, donde los ciudadanos se transforman gradualmente en rinocerontes. Ionesco fue una figura clave en el movimiento del Teatro del Absurdo junto con Beckett y Pinter.",
          de: "Eugène Ionescos 'Rhinoceros' (1959) ist eine Allegorie über Konformität und den Aufstieg des Faschismus, in der sich Bürger allmählich in Nashörner verwandeln. Ionesco war eine Schlüsselfigur in der Bewegung des Theaters des Absurden neben Beckett und Pinter.",
          nl: "Eugène Ionesco's 'Rhinoceros' (1959) is een allegorie over conformiteit en de opkomst van het fascisme, waarbij burgers geleidelijk transformeren in neushoorns. Ionesco was een sleutelfiguur in de Theater van het Absurde beweging naast Beckett en Pinter."
        }
      },
      {
        question: {
          en: "Which Indian author's novel 'Midnight's Children' employs magical realism to chronicle India's partition?",
          es: "¿La novela 'Los hijos de la medianoche' de qué autor indio emplea el realismo mágico para narrar la partición de India?",
          de: "Welches indischen Autors Roman 'Mitternachtskinder' verwendet magischen Realismus, um Indiens Teilung zu chroniken?",
          nl: "Welke Indiase auteur's roman 'Midnight's Children' gebruikt magisch realisme om India's deling te beschrijven?"
        },
        options: [
          {en: "R.K. Narayan", es: "R.K. Narayan", de: "R.K. Narayan", nl: "R.K. Narayan"},
          {en: "Salman Rushdie", es: "Salman Rushdie", de: "Salman Rushdie", nl: "Salman Rushdie"},
          {en: "Vikram Seth", es: "Vikram Seth", de: "Vikram Seth", nl: "Vikram Seth"},
          {en: "Anita Desai", es: "Anita Desai", de: "Anita Desai", nl: "Anita Desai"}
        ],
        correct: 1,
        explanation: {
          en: "Salman Rushdie's 'Midnight's Children' (1981) won the Booker Prize and chronicles India's history through the telepathic protagonist Saleem Sinai, born at the exact moment of India's independence. The novel pioneered postcolonial magical realism.",
          es: "'Los hijos de la medianoche' (1981) de Salman Rushdie ganó el Premio Booker y narra la historia de India a través del protagonista telepático Saleem Sinai, nacido en el momento exacto de la independencia de India. La novela fue pionera del realismo mágico poscolonial.",
          de: "Salman Rushdies 'Mitternachtskinder' (1981) gewann den Booker Prize und erzählt Indiens Geschichte durch den telepathischen Protagonisten Saleem Sinai, geboren im exakten Moment von Indiens Unabhängigkeit. Der Roman war wegweisend für den postkolonialen magischen Realismus.",
          nl: "Salman Rushdie's 'Midnight's Children' (1981) won de Booker Prize en vertelt India's geschiedenis door de telepathische protagonist Saleem Sinai, geboren op het exacte moment van India's onafhankelijkheid. De roman was baanbrekend voor postkoloniaal magisch realisme."
        }
      },
      {
        question: {
          en: "Which Belgian author wrote 'The Blue Lotus', featuring the detective comic character Tintin?",
          es: "¿Qué autor belga escribió 'El loto azul', protagonizado por el personaje de cómic detective Tintín?",
          de: "Welcher belgische Autor schrieb 'Der blaue Lotus' mit der Detektiv-Comic-Figur Tim?",
          nl: "Welke Belgische auteur schreef 'De Blauwe Lotus', met het detective stripfiguur Kuifje?"
        },
        options: [
          {en: "Hergé", es: "Hergé", de: "Hergé", nl: "Hergé"},
          {en: "André Franquin", es: "André Franquin", de: "André Franquin", nl: "André Franquin"},
          {en: "Morris", es: "Morris", de: "Morris", nl: "Morris"},
          {en: "Peyo", es: "Peyo", de: "Peyo", nl: "Peyo"}
        ],
        correct: 0,
        explanation: {
          en: "Hergé (Georges Remi) created Tintin and wrote 'The Blue Lotus' (1936), which is considered one of the finest Tintin adventures. Hergé revolutionized comic book storytelling with his 'clear line' artistic style and meticulous research.",
          es: "Hergé (Georges Remi) creó a Tintín y escribió 'El loto azul' (1936), que se considera una de las mejores aventuras de Tintín. Hergé revolucionó la narrativa de cómics con su estilo artístico de 'línea clara' y investigación meticulosa.",
          de: "Hergé (Georges Remi) schuf Tim und schrieb 'Der blaue Lotus' (1936), der als eines der besten Tim-Abenteuer gilt. Hergé revolutionierte das Comic-Erzählen mit seinem 'klare Linie' Kunststil und akribischer Recherche.",
          nl: "Hergé (Georges Remi) creëerde Kuifje en schreef 'De Blauwe Lotus' (1936), die wordt beschouwd als een van de beste Kuifje-avonturen. Hergé revolutioneerde stripverhalen met zijn 'klare lijn' artistieke stijl en nauwgezet onderzoek."
        }
      },
      {
        question: {
          en: "Which ancient Greek epic poet is traditionally credited with composing both 'The Iliad' and 'The Odyssey'?",
          es: "¿A qué poeta épico griego antiguo se le atribuye tradicionalmente la composición tanto de 'La Ilíada' como de 'La Odisea'?",
          de: "Welchem antiken griechischen Epiker wird traditionell die Komposition sowohl der 'Ilias' als auch der 'Odyssee' zugeschrieben?",
          nl: "Welke oude Griekse epische dichter wordt traditioneel gecrediteerd met het componeren van zowel 'De Ilias' als 'De Odyssee'?"
        },
        options: [
          {en: "Hesiod", es: "Hesíodo", de: "Hesiod", nl: "Hesiodus"},
          {en: "Homer", es: "Homero", de: "Homer", nl: "Homerus"},
          {en: "Pindar", es: "Píndaro", de: "Pindar", nl: "Pindarus"},
          {en: "Aeschylus", es: "Esquilo", de: "Aischylos", nl: "Aeschylus"}
        ],
        correct: 1,
        explanation: {
          en: "Homer is traditionally credited with composing both epic poems, though modern scholarship debates whether they were written by one person or represent oral traditions. The 'Homeric Question' remains one of literature's great mysteries.",
          es: "A Homero se le atribuye tradicionalmente la composición de ambos poemas épicos, aunque la erudición moderna debate si fueron escritos por una persona o representan tradiciones orales. La 'Cuestión Homérica' sigue siendo uno de los grandes misterios de la literatura.",
          de: "Homer wird traditionell die Komposition beider Epen zugeschrieben, obwohl die moderne Wissenschaft debattiert, ob sie von einer Person geschrieben wurden oder mündliche Traditionen repräsentieren. Die 'Homerische Frage' bleibt eines der großen Mysterien der Literatur.",
          nl: "Homerus wordt traditioneel gecrediteerd met het componeren van beide epische gedichten, hoewel moderne wetenschap debatteert of ze door één persoon werden geschreven of mondelinge tradities vertegenwoordigen. De 'Homerische Kwestie' blijft een van de grote mysteries van de literatuur."
        }
      },
      {
        question: {
          en: "Which Scottish author's 'The Strange Case of Dr. Jekyll and Mr. Hyde' explores the duality of human nature?",
          es: "¿'El extraño caso del Dr. Jekyll y Mr. Hyde' de qué autor escocés explora la dualidad de la naturaleza humana?",
          de: "Welches schottischen Autors 'Der seltsame Fall des Dr. Jekyll und Mr. Hyde' erforscht die Dualität der menschlichen Natur?",
          nl: "Welke Schotse auteur's 'Het vreemde geval van Dr. Jekyll en Mr. Hyde' verkent de dualiteit van de menselijke natuur?"
        },
        options: [
          {en: "Arthur Conan Doyle", es: "Arthur Conan Doyle", de: "Arthur Conan Doyle", nl: "Arthur Conan Doyle"},
          {en: "Robert Louis Stevenson", es: "Robert Louis Stevenson", de: "Robert Louis Stevenson", nl: "Robert Louis Stevenson"},
          {en: "Walter Scott", es: "Walter Scott", de: "Walter Scott", nl: "Walter Scott"},
          {en: "James Hogg", es: "James Hogg", de: "James Hogg", nl: "James Hogg"}
        ],
        correct: 1,
        explanation: {
          en: "Robert Louis Stevenson wrote 'The Strange Case of Dr. Jekyll and Mr. Hyde' (1886) after reportedly having a nightmare. The novella became a classic exploration of the good and evil duality within human nature and Victorian society's hypocrisy.",
          es: "Robert Louis Stevenson escribió 'El extraño caso del Dr. Jekyll y Mr. Hyde' (1886) después de supuestamente tener una pesadilla. La novela corta se convirtió en una exploración clásica de la dualidad del bien y el mal dentro de la naturaleza humana y la hipocresía de la sociedad victoriana.",
          de: "Robert Louis Stevenson schrieb 'Der seltsame Fall des Dr. Jekyll und Mr. Hyde' (1886) angeblich nach einem Albtraum. Die Novelle wurde zu einer klassischen Erforschung der Gut-Böse-Dualität in der menschlichen Natur und der Heuchelei der viktorianischen Gesellschaft.",
          nl: "Robert Louis Stevenson schreef 'Het vreemde geval van Dr. Jekyll en Mr. Hyde' (1886) naar verluidt na een nachtmerrie. De novelle werd een klassieke verkenning van de goed-kwaad dualiteit binnen de menselijke natuur en de hypocrisie van de Victoriaanse samenleving."
        }
      },
      {
        question: {
          en: "Which modernist poet wrote 'The Hollow Men' with the famous line 'This is the way the world ends / Not with a bang but a whimper'?",
          es: "¿Qué poeta modernista escribió 'Los hombres huecos' con la famosa línea 'Así es como termina el mundo / No con una explosión sino con un gemido'?",
          de: "Welcher modernistische Dichter schrieb 'Die hohlen Männer' mit der berühmten Zeile 'So endet die Welt / Nicht mit einem Knall, sondern mit einem Wimmern'?",
          nl: "Welke modernistische dichter schreef 'The Hollow Men' met de beroemde regel 'This is the way the world ends / Not with a bang but a whimper'?"
        },
        options: [
          {en: "W.H. Auden", es: "W.H. Auden", de: "W.H. Auden", nl: "W.H. Auden"},
          {en: "T.S. Eliot", es: "T.S. Eliot", de: "T.S. Eliot", nl: "T.S. Eliot"},
          {en: "Ezra Pound", es: "Ezra Pound", de: "Ezra Pound", nl: "Ezra Pound"},
          {en: "William Butler Yeats", es: "William Butler Yeats", de: "William Butler Yeats", nl: "William Butler Yeats"}
        ],
        correct: 1,
        explanation: {
          en: "T.S. Eliot wrote 'The Hollow Men' (1925), which ends with these famous lines. The poem reflects post-World War I disillusionment and spiritual emptiness, becoming one of the most quoted endings in modern poetry.",
          es: "T.S. Eliot escribió 'Los hombres huecos' (1925), que termina con estas famosas líneas. El poema refleja la desilusión posterior a la Primera Guerra Mundial y el vacío espiritual, convirtiéndose en uno de los finales más citados en la poesía moderna.",
          de: "T.S. Eliot schrieb 'Die hohlen Männer' (1925), das mit diesen berühmten Zeilen endet. Das Gedicht spiegelt die Desillusionierung nach dem Ersten Weltkrieg und spirituelle Leere wider und wurde zu einem der meistzitierten Enden in der modernen Dichtung.",
          nl: "T.S. Eliot schreef 'The Hollow Men' (1925), dat eindigt met deze beroemde regels. Het gedicht reflecteert de desillusionment na de Eerste Wereldoorlog en spirituele leegte, en werd een van de meest geciteerde eindes in moderne poëzie."
        }
      },
      {
        question: {
          en: "Which Spanish author's 'Don Quixote' is often considered the first modern novel?",
          es: "¿'Don Quijote' de qué autor español se considera a menudo la primera novela moderna?",
          de: "Welches spanischen Autors 'Don Quixote' wird oft als der erste moderne Roman betrachtet?",
          nl: "Welke Spaanse auteur's 'Don Quichot' wordt vaak beschouwd als de eerste moderne roman?"
        },
        options: [
          {en: "Lope de Vega", es: "Lope de Vega", de: "Lope de Vega", nl: "Lope de Vega"},
          {en: "Miguel de Cervantes", es: "Miguel de Cervantes", de: "Miguel de Cervantes", nl: "Miguel de Cervantes"},
          {en: "Francisco de Quevedo", es: "Francisco de Quevedo", de: "Francisco de Quevedo", nl: "Francisco de Quevedo"},
          {en: "Pedro Calderón de la Barca", es: "Pedro Calderón de la Barca", de: "Pedro Calderón de la Barca", nl: "Pedro Calderón de la Barca"}
        ],
        correct: 1,
        explanation: {
          en: "Miguel de Cervantes published 'Don Quixote' in two parts (1605, 1615), pioneering techniques like meta-fiction, unreliable narration, and psychological depth that define the modern novel. It's considered the first work of modern Western literature.",
          es: "Miguel de Cervantes publicó 'Don Quijote' en dos partes (1605, 1615), siendo pionero en técnicas como la meta-ficción, narración no confiable y profundidad psicológica que definen la novela moderna. Se considera la primera obra de la literatura occidental moderna.",
          de: "Miguel de Cervantes veröffentlichte 'Don Quixote' in zwei Teilen (1605, 1615) und war Pionier für Techniken wie Meta-Fiktion, unzuverlässige Erzählung und psychologische Tiefe, die den modernen Roman definieren. Es gilt als das erste Werk der modernen westlichen Literatur.",
          nl: "Miguel de Cervantes publiceerde 'Don Quichot' in twee delen (1605, 1615), baanbrekend in technieken zoals meta-fictie, onbetrouwbare vertelling en psychologische diepte die de moderne roman definiëren. Het wordt beschouwd als het eerste werk van moderne westerse literatuur."
        }
      },
      {
        question: {
          en: "Which Russian author's 'Notes from Underground' is considered a precursor to existentialist literature?",
          es: "¿'Memorias del subsuelo' de qué autor ruso se considera un precursor de la literatura existencialista?",
          de: "Welches russischen Autors 'Aufzeichnungen aus dem Untergrund' wird als Vorläufer der existentialistischen Literatur betrachtet?",
          nl: "Welke Russische auteur's 'Aantekeningen uit het ondergrondse' wordt beschouwd als een voorloper van de existentialistische literatuur?"
        },
        options: [
          {en: "Leo Tolstoy", es: "León Tolstói", de: "Leo Tolstoj", nl: "Leo Tolstoj"},
          {en: "Fyodor Dostoevsky", es: "Fiódor Dostoyevski", de: "Fjodor Dostojewski", nl: "Fjodor Dostojevski"},
          {en: "Ivan Turgenev", es: "Iván Turguénev", de: "Iwan Turgenew", nl: "Ivan Toergenjev"},
          {en: "Anton Chekhov", es: "Antón Chéjov", de: "Anton Tschechow", nl: "Anton Tsjechov"}
        ],
        correct: 1,
        explanation: {
          en: "Fyodor Dostoevsky's 'Notes from Underground' (1864) features an alienated narrator wrestling with free will, consciousness, and irrationality. The work's psychological introspection and themes of existential angst heavily influenced later existentialist philosophers like Sartre and Camus.",
          es: "'Memorias del subsuelo' (1864) de Fiódor Dostoyevski presenta un narrador alienado luchando con el libre albedrío, la conciencia y la irracionalidad. La introspección psicológica de la obra y los temas de angustia existencial influyeron mucho en filósofos existencialistas posteriores como Sartre y Camus.",
          de: "Fjodor Dostojewskis 'Aufzeichnungen aus dem Untergrund' (1864) zeigt einen entfremdeten Erzähler, der mit freiem Willen, Bewusstsein und Irrationalität ringt. Die psychologische Selbstbetrachtung des Werks und Themen existentieller Angst beeinflussten spätere existentialistische Philosophen wie Sartre und Camus stark.",
          nl: "Fjodor Dostojevski's 'Aantekeningen uit het ondergrondse' (1864) toont een vervreemde verteller die worstelt met vrije wil, bewustzijn en irrationaliteit. De psychologische introspectie van het werk en thema's van existentiële angst beïnvloedden latere existentialistische filosofen zoals Sartre en Camus sterk."
        }
      },
      {
        question: {
          en: "Which Canadian author's 'The Handmaid's Tale' depicts a dystopian theocracy where women's rights are severely restricted?",
          es: "¿'El cuento de la criada' de qué autora canadiense retrata una teocracia distópica donde los derechos de las mujeres están severamente restringidos?",
          de: "Welche kanadischen Autorin's 'Der Report der Magd' zeigt eine dystopische Theokratie, in der Frauenrechte stark eingeschränkt sind?",
          nl: "Welke Canadese auteur's 'The Handmaid's Tale' toont een dystopische theocratie waar vrouwenrechten zwaar beperkt zijn?"
        },
        options: [
          {en: "Alice Munro", es: "Alice Munro", de: "Alice Munro", nl: "Alice Munro"},
          {en: "Margaret Atwood", es: "Margaret Atwood", de: "Margaret Atwood", nl: "Margaret Atwood"},
          {en: "Carol Shields", es: "Carol Shields", de: "Carol Shields", nl: "Carol Shields"},
          {en: "Mordecai Richler", es: "Mordecai Richler", de: "Mordecai Richler", nl: "Mordecai Richler"}
        ],
        correct: 1,
        explanation: {
          en: "Margaret Atwood's 'The Handmaid's Tale' (1985) depicts the Republic of Gilead, where fertile women are forced into reproductive servitude. The novel's themes of authoritarianism and women's rights have made it increasingly relevant in contemporary political discourse.",
          es: "'El cuento de la criada' (1985) de Margaret Atwood retrata la República de Gilead, donde las mujeres fértiles son obligadas a la servidumbre reproductiva. Los temas de autoritarismo y derechos de las mujeres de la novela la han hecho cada vez más relevante en el discurso político contemporáneo.",
          de: "Margaret Atwoods 'Der Report der Magd' (1985) zeigt die Republik Gilead, wo fruchtbare Frauen zur reproduktiven Knechtschaft gezwungen werden. Die Themen des Romans über Autoritarismus und Frauenrechte haben ihn im zeitgenössischen politischen Diskurs zunehmend relevant gemacht.",
          nl: "Margaret Atwood's 'The Handmaid's Tale' (1985) toont de Republiek Gilead, waar vruchtbare vrouwen gedwongen worden tot reproductieve dienstbaarheid. De thema's van de roman over autoritarisme en vrouwenrechten hebben het steeds relevanter gemaakt in het hedendaagse politieke discours."
        }
      },
      {
        question: {
          en: "Which Irish playwright wrote 'Waiting for Godot', a landmark work of absurdist theatre?",
          es: "¿Qué dramaturgo irlandés escribió 'Esperando a Godot', una obra emblemática del teatro absurdista?",
          de: "Welcher irische Dramatiker schrieb 'Warten auf Godot', ein Meilenstein des absurden Theaters?",
          nl: "Welke Ierse toneelschrijver schreef 'Wachten op Godot', een mijlpaal van het absurdistische theater?"
        },
        options: [
          {en: "Oscar Wilde", es: "Oscar Wilde", de: "Oscar Wilde", nl: "Oscar Wilde"},
          {en: "Samuel Beckett", es: "Samuel Beckett", de: "Samuel Beckett", nl: "Samuel Beckett"},
          {en: "Sean O'Casey", es: "Sean O'Casey", de: "Sean O'Casey", nl: "Sean O'Casey"},
          {en: "Brian Friel", es: "Brian Friel", de: "Brian Friel", nl: "Brian Friel"}
        ],
        correct: 1,
        explanation: {
          en: "Samuel Beckett wrote 'Waiting for Godot' (1953), originally in French as 'En attendant Godot'. The play's minimalist plot and existential themes made it a cornerstone of absurdist theatre and earned Beckett the Nobel Prize in Literature in 1969.",
          es: "Samuel Beckett escribió 'Esperando a Godot' (1953), originalmente en francés como 'En attendant Godot'. La trama minimalista de la obra y los temas existenciales la convirtieron en una piedra angular del teatro absurdista y le valieron a Beckett el Premio Nobel de Literatura en 1969.",
          de: "Samuel Beckett schrieb 'Warten auf Godot' (1953), ursprünglich auf Französisch als 'En attendant Godot'. Die minimalistische Handlung des Stücks und existentielle Themen machten es zu einem Grundpfeiler des absurden Theaters und brachten Beckett 1969 den Nobelpreis für Literatur ein.",
          nl: "Samuel Beckett schreef 'Wachten op Godot' (1953), oorspronkelijk in het Frans als 'En attendant Godot'. De minimalistische plot van het stuk en existentiële thema's maakten het een hoeksteen van het absurdistische theater en leverden Beckett de Nobelprijs voor Literatuur op in 1969."
        }
      },
      {
        question: {
          en: "Which Italian author wrote 'The Name of the Rose', a medieval murder mystery filled with literary and philosophical references?",
          es: "¿Qué autor italiano escribió 'El nombre de la rosa', un misterio de asesinato medieval lleno de referencias literarias y filosóficas?",
          de: "Welcher italienische Autor schrieb 'Der Name der Rose', einen mittelalterlichen Krimi voller literarischer und philosophischer Verweise?",
          nl: "Welke Italiaanse auteur schreef 'De Naam van de Roos', een middeleeuws moordmysterie vol literaire en filosofische verwijzingen?"
        },
        options: [
          {en: "Umberto Eco", es: "Umberto Eco", de: "Umberto Eco", nl: "Umberto Eco"},
          {en: "Italo Calvino", es: "Italo Calvino", de: "Italo Calvino", nl: "Italo Calvino"},
          {en: "Primo Levi", es: "Primo Levi", de: "Primo Levi", nl: "Primo Levi"},
          {en: "Leonardo Sciascia", es: "Leonardo Sciascia", de: "Leonardo Sciascia", nl: "Leonardo Sciascia"}
        ],
        correct: 0,
        explanation: {
          en: "Umberto Eco wrote 'The Name of the Rose' (1980), combining a detective story with semiotics, medieval history, and literary theory. The novel became an international bestseller and established Eco's reputation beyond academia.",
          es: "Umberto Eco escribió 'El nombre de la rosa' (1980), combinando una historia de detectives con semiótica, historia medieval y teoría literaria. La novela se convirtió en un bestseller internacional y estableció la reputación de Eco más allá del ámbito académico.",
          de: "Umberto Eco schrieb 'Der Name der Rose' (1980), der eine Detektivgeschichte mit Semiotik, mittelalterlicher Geschichte und Literaturtheorie kombiniert. Der Roman wurde ein internationaler Bestseller und etablierte Ecos Ruf über die Wissenschaft hinaus.",
          nl: "Umberto Eco schreef 'De Naam van de Roos' (1980), waarin een detectiveverhaal wordt gecombineerd met semiotiek, middeleeuwse geschiedenis en literatuurtheorie. De roman werd een internationale bestseller en vestigde Eco's reputatie buiten de academische wereld."
        }
      },
      {
        question: {
          en: "Which Nigerian author wrote 'Half of a Yellow Sun', chronicling the Biafran War?",
          es: "¿Qué autora nigeriana escribió 'Medio sol amarillo', narrando la Guerra de Biafra?",
          de: "Welche nigerianische Autorin schrieb 'Die Hälfte der Sonne', das den Biafra-Krieg erzählt?",
          nl: "Welke Nigeriaanse auteur schreef 'De Helft van een Gele Zon', over de Biafra-oorlog?"
        },
        options: [
          {en: "Chimamanda Ngozi Adichie", es: "Chimamanda Ngozi Adichie", de: "Chimamanda Ngozi Adichie", nl: "Chimamanda Ngozi Adichie"},
          {en: "Chinua Achebe", es: "Chinua Achebe", de: "Chinua Achebe", nl: "Chinua Achebe"},
          {en: "Buchi Emecheta", es: "Buchi Emecheta", de: "Buchi Emecheta", nl: "Buchi Emecheta"},
          {en: "Wole Soyinka", es: "Wole Soyinka", de: "Wole Soyinka", nl: "Wole Soyinka"}
        ],
        correct: 0,
        explanation: {
          en: "Chimamanda Ngozi Adichie wrote 'Half of a Yellow Sun' (2006), which won the Orange Prize for Fiction. The novel explores the Nigerian Civil War through multiple perspectives, examining themes of love, loyalty, and political upheaval.",
          es: "Chimamanda Ngozi Adichie escribió 'Medio sol amarillo' (2006), que ganó el Premio Orange de Ficción. La novela explora la Guerra Civil Nigeriana a través de múltiples perspectivas, examinando temas de amor, lealtad y agitación política.",
          de: "Chimamanda Ngozi Adichie schrieb 'Die Hälfte der Sonne' (2006), das den Orange Prize for Fiction gewann. Der Roman erforscht den nigerianischen Bürgerkrieg durch multiple Perspektiven und untersucht Themen wie Liebe, Loyalität und politischen Umbruch.",
          nl: "Chimamanda Ngozi Adichie schreef 'De Helft van een Gele Zon' (2006), dat de Orange Prize for Fiction won. De roman verkent de Nigeriaanse Burgeroorlog door meerdere perspectieven en onderzoekt thema's als liefde, loyaliteit en politieke omwenteling."
        }
      },
      {
        question: {
          en: "Which Welsh poet wrote 'Do not go gentle into that good night', a villanelle about facing death?",
          es: "¿Qué poeta galés escribió 'No entres dócilmente en esa buena noche', una villanela sobre enfrentar la muerte?",
          de: "Welcher walisische Dichter schrieb 'Do not go gentle into that good night', eine Villanelle über die Konfrontation mit dem Tod?",
          nl: "Welke Welshe dichter schreef 'Do not go gentle into that good night', een villanelle over de confrontatie met de dood?"
        },
        options: [
          {en: "Dylan Thomas", es: "Dylan Thomas", de: "Dylan Thomas", nl: "Dylan Thomas"},
          {en: "R.S. Thomas", es: "R.S. Thomas", de: "R.S. Thomas", nl: "R.S. Thomas"},
          {en: "W.H. Davies", es: "W.H. Davies", de: "W.H. Davies", nl: "W.H. Davies"},
          {en: "Edward Thomas", es: "Edward Thomas", de: "Edward Thomas", nl: "Edward Thomas"}
        ],
        correct: 0,
        explanation: {
          en: "Dylan Thomas wrote 'Do not go gentle into that good night' (1947) for his dying father. The villanelle's powerful refrain and imagery made it one of the most famous poems in English, celebrating resistance against death.",
          es: "Dylan Thomas escribió 'No entres dócilmente en esa buena noche' (1947) para su padre moribundo. El poderoso estribillo de la villanela y sus imágenes la convirtieron en uno de los poemas más famosos en inglés, celebrando la resistencia contra la muerte.",
          de: "Dylan Thomas schrieb 'Do not go gentle into that good night' (1947) für seinen sterbenden Vater. Der kraftvolle Refrain der Villanelle und ihre Bilder machten sie zu einem der berühmtesten Gedichte in englischer Sprache, das den Widerstand gegen den Tod feiert.",
          nl: "Dylan Thomas schreef 'Do not go gentle into that good night' (1947) voor zijn stervende vader. Het krachtige refrein van de villanelle en de beeldspraak maakten het een van de beroemdste gedichten in het Engels, dat verzet tegen de dood viert."
        }
      },
      {
        question: {
          en: "Which Australian author's novel 'The Secret River' explores colonial history and Indigenous displacement?",
          es: "¿La novela 'El río secreto' de qué autora australiana explora la historia colonial y el desplazamiento indígena?",
          de: "Welche australischen Autorin's Roman 'The Secret River' erforscht Kolonialgeschichte und indigene Vertreibung?",
          nl: "Welke Australische auteur's roman 'The Secret River' verkent de koloniale geschiedenis en de verplaatsing van inheemse volkeren?"
        },
        options: [
          {en: "Kate Grenville", es: "Kate Grenville", de: "Kate Grenville", nl: "Kate Grenville"},
          {en: "Christina Stead", es: "Christina Stead", de: "Christina Stead", nl: "Christina Stead"},
          {en: "Helen Garner", es: "Helen Garner", de: "Helen Garner", nl: "Helen Garner"},
          {en: "Colleen McCullough", es: "Colleen McCullough", de: "Colleen McCullough", nl: "Colleen McCullough"}
        ],
        correct: 0,
        explanation: {
          en: "Kate Grenville wrote 'The Secret River' (2005), examining the violent frontier conflict between European settlers and Aboriginal Australians in early 19th century New South Wales. The novel won the Commonwealth Writers' Prize.",
          es: "Kate Grenville escribió 'El río secreto' (2005), examinando el violento conflicto fronterizo entre colonos europeos y aborígenes australianos en Nueva Gales del Sur a principios del siglo XIX. La novela ganó el Premio de Escritores del Commonwealth.",
          de: "Kate Grenville schrieb 'The Secret River' (2005), das den gewaltsamen Grenzkonflikt zwischen europäischen Siedlern und australischen Ureinwohnern im frühen 19. Jahrhundert in New South Wales untersucht. Der Roman gewann den Commonwealth Writers' Prize.",
          nl: "Kate Grenville schreef 'The Secret River' (2005), waarin het gewelddadige grensconflict tussen Europese kolonisten en Aboriginals in het vroege 19e-eeuwse New South Wales wordt onderzocht. De roman won de Commonwealth Writers' Prize."
        }
      },
      {
        question: {
          en: "Which South African author wrote 'Disgrace', exploring post-apartheid racial tensions?",
          es: "¿Qué autor sudafricano escribió 'Desgracia', explorando las tensiones raciales post-apartheid?",
          de: "Welcher südafrikanische Autor schrieb 'Schande', das die rassischen Spannungen nach der Apartheid erforscht?",
          nl: "Welke Zuid-Afrikaanse auteur schreef 'Disgrace', waarin de raciale spanningen na de apartheid worden verkend?"
        },
        options: [
          {en: "J.M. Coetzee", es: "J.M. Coetzee", de: "J.M. Coetzee", nl: "J.M. Coetzee"},
          {en: "Nadine Gordimer", es: "Nadine Gordimer", de: "Nadine Gordimer", nl: "Nadine Gordimer"},
          {en: "André Brink", es: "André Brink", de: "André Brink", nl: "André Brink"},
          {en: "Alan Paton", es: "Alan Paton", de: "Alan Paton", nl: "Alan Paton"}
        ],
        correct: 0,
        explanation: {
          en: "J.M. Coetzee wrote 'Disgrace' (1999), which won the Booker Prize. The novel examines violence, power, and reconciliation in post-apartheid South Africa. Coetzee won the Nobel Prize in Literature in 2003.",
          es: "J.M. Coetzee escribió 'Desgracia' (1999), que ganó el Premio Booker. La novela examina la violencia, el poder y la reconciliación en Sudáfrica post-apartheid. Coetzee ganó el Premio Nobel de Literatura en 2003.",
          de: "J.M. Coetzee schrieb 'Schande' (1999), das den Booker Prize gewann. Der Roman untersucht Gewalt, Macht und Versöhnung im Südafrika nach der Apartheid. Coetzee gewann 2003 den Nobelpreis für Literatur.",
          nl: "J.M. Coetzee schreef 'Disgrace' (1999), dat de Booker Prize won. De roman onderzoekt geweld, macht en verzoening in post-apartheid Zuid-Afrika. Coetzee won de Nobelprijs voor Literatuur in 2003."
        }
      },
      {
        question: {
          en: "Which Brazilian author wrote 'The Posthumous Memoirs of Brás Cubas', narrated by a dead man?",
          es: "¿Qué autor brasileño escribió 'Memorias póstumas de Brás Cubas', narrado por un hombre muerto?",
          de: "Welcher brasilianische Autor schrieb 'Posthume Erinnerungen des Brás Cubas', erzählt von einem Toten?",
          nl: "Welke Braziliaanse auteur schreef 'De Postume Memoires van Brás Cubas', verteld door een dode man?"
        },
        options: [
          {en: "Jorge Amado", es: "Jorge Amado", de: "Jorge Amado", nl: "Jorge Amado"},
          {en: "Machado de Assis", es: "Machado de Assis", de: "Machado de Assis", nl: "Machado de Assis"},
          {en: "Clarice Lispector", es: "Clarice Lispector", de: "Clarice Lispector", nl: "Clarice Lispector"},
          {en: "Paulo Coelho", es: "Paulo Coelho", de: "Paulo Coelho", nl: "Paulo Coelho"}
        ],
        correct: 1,
        explanation: {
          en: "Machado de Assis wrote 'The Posthumous Memoirs of Brás Cubas' (1881), a pioneering work of literary realism narrated by a deceased protagonist. This innovative novel is considered a masterpiece of Brazilian literature.",
          es: "Machado de Assis escribió 'Memorias póstumas de Brás Cubas' (1881), una obra pionera del realismo literario narrada por un protagonista fallecido. Esta novela innovadora se considera una obra maestra de la literatura brasileña.",
          de: "Machado de Assis schrieb 'Posthume Erinnerungen des Brás Cubas' (1881), ein wegweisendes Werk des literarischen Realismus, das von einem verstorbenen Protagonisten erzählt wird. Dieser innovative Roman gilt als Meisterwerk der brasilianischen Literatur.",
          nl: "Machado de Assis schreef 'De Postume Memoires van Brás Cubas' (1881), een baanbrekend werk van literair realisme verteld door een overleden protagonist. Deze innovatieve roman wordt beschouwd als een meesterwerk van de Braziliaanse literatuur."
        }
      },
      {
        question: {
          en: "Which Filipino author wrote 'Noli Me Tangere', a novel that inspired the Philippine independence movement?",
          es: "¿Qué autor filipino escribió 'Noli Me Tangere', una novela que inspiró el movimiento de independencia filipino?",
          de: "Welcher philippinische Autor schrieb 'Noli Me Tangere', einen Roman, der die philippinische Unabhängigkeitsbewegung inspirierte?",
          nl: "Welke Filipijnse auteur schreef 'Noli Me Tangere', een roman die de Filipijnse onafhankelijkheidsbeweging inspireerde?"
        },
        options: [
          {en: "José Rizal", es: "José Rizal", de: "José Rizal", nl: "José Rizal"},
          {en: "Nick Joaquin", es: "Nick Joaquin", de: "Nick Joaquin", nl: "Nick Joaquin"},
          {en: "F. Sionil José", es: "F. Sionil José", de: "F. Sionil José", nl: "F. Sionil José"},
          {en: "Carlos Bulosan", es: "Carlos Bulosan", de: "Carlos Bulosan", nl: "Carlos Bulosan"}
        ],
        correct: 0,
        explanation: {
          en: "José Rizal wrote 'Noli Me Tangere' (1887), exposing Spanish colonial abuses in the Philippines. The novel became a catalyst for revolution and Rizal became a national hero, though he was executed by Spanish authorities in 1896.",
          es: "José Rizal escribió 'Noli Me Tangere' (1887), exponiendo los abusos coloniales españoles en Filipinas. La novela se convirtió en un catalizador de la revolución y Rizal se convirtió en héroe nacional, aunque fue ejecutado por las autoridades españolas en 1896.",
          de: "José Rizal schrieb 'Noli Me Tangere' (1887), das die spanischen Kolonialverbrechen auf den Philippinen aufdeckte. Der Roman wurde zum Katalysator für die Revolution und Rizal wurde zum Nationalhelden, obwohl er 1896 von den spanischen Behörden hingerichtet wurde.",
          nl: "José Rizal schreef 'Noli Me Tangere' (1887), waarin de Spaanse koloniale misstanden op de Filipijnen werden onthuld. De roman werd een katalysator voor revolutie en Rizal werd een nationale held, hoewel hij in 1896 door Spaanse autoriteiten werd geëxecuteerd."
        }
      },
      {
        question: {
          en: "Which Korean author wrote 'The Vegetarian', exploring themes of violence and bodily autonomy?",
          es: "¿Qué autora coreana escribió 'La vegetariana', explorando temas de violencia y autonomía corporal?",
          de: "Welche koreanische Autorin schrieb 'Die Vegetarierin', die Themen von Gewalt und körperlicher Autonomie erforscht?",
          nl: "Welke Koreaanse auteur schreef 'De Vegetariër', waarin thema's van geweld en lichamelijke autonomie worden verkend?"
        },
        options: [
          {en: "Han Kang", es: "Han Kang", de: "Han Kang", nl: "Han Kang"},
          {en: "Kyung-Sook Shin", es: "Kyung-Sook Shin", de: "Kyung-Sook Shin", nl: "Kyung-Sook Shin"},
          {en: "Yi Mun-yol", es: "Yi Mun-yol", de: "Yi Mun-yol", nl: "Yi Mun-yol"},
          {en: "Hwang Sun-won", es: "Hwang Sun-won", de: "Hwang Sun-won", nl: "Hwang Sun-won"}
        ],
        correct: 0,
        explanation: {
          en: "Han Kang wrote 'The Vegetarian' (2007), which won the Man Booker International Prize in 2016. The surreal novel explores a woman's radical rejection of violence through her decision to stop eating meat, disturbing her conventional family.",
          es: "Han Kang escribió 'La vegetariana' (2007), que ganó el Premio Man Booker Internacional en 2016. La novela surrealista explora el rechazo radical de una mujer a la violencia a través de su decisión de dejar de comer carne, perturbando a su familia convencional.",
          de: "Han Kang schrieb 'Die Vegetarierin' (2007), das 2016 den Man Booker International Prize gewann. Der surreale Roman erforscht die radikale Ablehnung einer Frau von Gewalt durch ihre Entscheidung, kein Fleisch mehr zu essen, was ihre konventionelle Familie verstört.",
          nl: "Han Kang schreef 'De Vegetariër' (2007), dat in 2016 de Man Booker International Prize won. De surrealistische roman verkent de radicale afwijzing van geweld door een vrouw via haar beslissing om te stoppen met het eten van vlees, wat haar conventionele familie verstoort."
        }
      },
      {
        question: {
          en: "Which Egyptian author won the Nobel Prize for his Cairo Trilogy and other works depicting Egyptian society?",
          es: "¿Qué autor egipcio ganó el Premio Nobel por su Trilogía de El Cairo y otras obras que retratan la sociedad egipcia?",
          de: "Welcher ägyptische Autor gewann den Nobelpreis für seine Kairo-Trilogie und andere Werke, die die ägyptische Gesellschaft darstellen?",
          nl: "Welke Egyptische auteur won de Nobelprijs voor zijn Caïro Trilogie en andere werken die de Egyptische samenleving uitbeelden?"
        },
        options: [
          {en: "Naguib Mahfouz", es: "Naguib Mahfouz", de: "Naguib Mahfouz", nl: "Naguib Mahfouz"},
          {en: "Taha Hussein", es: "Taha Hussein", de: "Taha Hussein", nl: "Taha Hussein"},
          {en: "Tawfiq al-Hakim", es: "Tawfiq al-Hakim", de: "Tawfiq al-Hakim", nl: "Tawfiq al-Hakim"},
          {en: "Yusuf Idris", es: "Yusuf Idris", de: "Yusuf Idris", nl: "Yusuf Idris"}
        ],
        correct: 0,
        explanation: {
          en: "Naguib Mahfouz won the Nobel Prize in Literature in 1988, the first Arabic-language writer to receive the honor. His Cairo Trilogy chronicles Egyptian life from 1917 to 1944, combining realism with philosophical depth.",
          es: "Naguib Mahfouz ganó el Premio Nobel de Literatura en 1988, el primer escritor en lengua árabe en recibir el honor. Su Trilogía de El Cairo narra la vida egipcia de 1917 a 1944, combinando realismo con profundidad filosófica.",
          de: "Naguib Mahfouz gewann 1988 den Nobelpreis für Literatur, der erste arabischsprachige Schriftsteller, der diese Ehre erhielt. Seine Kairo-Trilogie erzählt das ägyptische Leben von 1917 bis 1944 und verbindet Realismus mit philosophischer Tiefe.",
          nl: "Naguib Mahfouz won de Nobelprijs voor Literatuur in 1988, de eerste Arabisch-talige schrijver die deze eer ontving. Zijn Caïro Trilogie vertelt het Egyptische leven van 1917 tot 1944, waarbij realisme wordt gecombineerd met filosofische diepte."
        }
      },
      {
        question: {
          en: "Which Caribbean author wrote 'Wide Sargasso Sea', a prequel to 'Jane Eyre' told from Bertha's perspective?",
          es: "¿Qué autora caribeña escribió 'Ancho mar de los Sargazos', una precuela de 'Jane Eyre' contada desde la perspectiva de Bertha?",
          de: "Welche karibische Autorin schrieb 'Wide Sargasso Sea', ein Prequel zu 'Jane Eyre' aus Berthas Perspektive erzählt?",
          nl: "Welke Caribische auteur schreef 'Wide Sargasso Sea', een prequel van 'Jane Eyre' verteld vanuit Bertha's perspectief?"
        },
        options: [
          {en: "Jean Rhys", es: "Jean Rhys", de: "Jean Rhys", nl: "Jean Rhys"},
          {en: "Jamaica Kincaid", es: "Jamaica Kincaid", de: "Jamaica Kincaid", nl: "Jamaica Kincaid"},
          {en: "V.S. Naipaul", es: "V.S. Naipaul", de: "V.S. Naipaul", nl: "V.S. Naipaul"},
          {en: "Derek Walcott", es: "Derek Walcott", de: "Derek Walcott", nl: "Derek Walcott"}
        ],
        correct: 0,
        explanation: {
          en: "Jean Rhys wrote 'Wide Sargasso Sea' (1966), giving voice to the 'madwoman in the attic' from 'Jane Eyre'. The novel explores colonialism, race, and gender through the story of Antoinette Cosway in Jamaica.",
          es: "Jean Rhys escribió 'Ancho mar de los Sargazos' (1966), dando voz a la 'mujer loca del ático' de 'Jane Eyre'. La novela explora el colonialismo, la raza y el género a través de la historia de Antoinette Cosway en Jamaica.",
          de: "Jean Rhys schrieb 'Wide Sargasso Sea' (1966), das der 'verrückten Frau auf dem Dachboden' aus 'Jane Eyre' eine Stimme gibt. Der Roman erforscht Kolonialismus, Rasse und Geschlecht durch die Geschichte von Antoinette Cosway in Jamaika.",
          nl: "Jean Rhys schreef 'Wide Sargasso Sea' (1966), waarin een stem wordt gegeven aan de 'gekke vrouw op zolder' uit 'Jane Eyre'. De roman verkent kolonialisme, ras en gender door het verhaal van Antoinette Cosway in Jamaica."
        }
      },
      {
        question: {
          en: "Which Swiss author wrote 'The Visit', a tragicomic play about justice and revenge?",
          es: "¿Qué autor suizo escribió 'La visita', una obra tragicómica sobre justicia y venganza?",
          de: "Welcher Schweizer Autor schrieb 'Der Besuch der alten Dame', ein tragikomisches Stück über Gerechtigkeit und Rache?",
          nl: "Welke Zwitserse auteur schreef 'The Visit', een tragikomisch toneelstuk over gerechtigheid en wraak?"
        },
        options: [
          {en: "Max Frisch", es: "Max Frisch", de: "Max Frisch", nl: "Max Frisch"},
          {en: "Friedrich Dürrenmatt", es: "Friedrich Dürrenmatt", de: "Friedrich Dürrenmatt", nl: "Friedrich Dürrenmatt"},
          {en: "Hermann Hesse", es: "Hermann Hesse", de: "Hermann Hesse", nl: "Hermann Hesse"},
          {en: "Robert Walser", es: "Robert Walser", de: "Robert Walser", nl: "Robert Walser"}
        ],
        correct: 1,
        explanation: {
          en: "Friedrich Dürrenmatt wrote 'The Visit' (1956), a dark comedy exploring moral corruption. A wealthy woman returns to her impoverished hometown offering prosperity in exchange for the murder of the man who wronged her decades earlier.",
          es: "Friedrich Dürrenmatt escribió 'La visita' (1956), una comedia negra que explora la corrupción moral. Una mujer rica regresa a su pueblo natal empobrecido ofreciendo prosperidad a cambio del asesinato del hombre que la perjudicó décadas antes.",
          de: "Friedrich Dürrenmatt schrieb 'Der Besuch der alten Dame' (1956), eine schwarze Komödie, die moralische Korruption erforscht. Eine reiche Frau kehrt in ihre verarmte Heimatstadt zurück und bietet Wohlstand im Austausch für den Mord an dem Mann, der ihr Jahrzehnte zuvor Unrecht tat.",
          nl: "Friedrich Dürrenmatt schreef 'The Visit' (1956), een zwarte komedie die morele corruptie verkent. Een rijke vrouw keert terug naar haar verarmde geboorteplaats en biedt welvaart in ruil voor de moord op de man die haar decennia eerder onrecht aandeed."
        }
      },
      {
        question: {
          en: "Which Iraqi poet is known as 'The Father of Free Verse' in Arabic poetry?",
          es: "¿Qué poeta iraquí es conocido como 'El Padre del Verso Libre' en la poesía árabe?",
          de: "Welcher irakische Dichter ist als 'Der Vater des freien Verses' in der arabischen Poesie bekannt?",
          nl: "Welke Iraakse dichter staat bekend als 'De Vader van het Vrije Vers' in de Arabische poëzie?"
        },
        options: [
          {en: "Badr Shakir al-Sayyab", es: "Badr Shakir al-Sayyab", de: "Badr Shakir al-Sayyab", nl: "Badr Shakir al-Sayyab"},
          {en: "Mahmoud Darwish", es: "Mahmoud Darwish", de: "Mahmoud Darwish", nl: "Mahmoud Darwish"},
          {en: "Adonis", es: "Adonis", de: "Adonis", nl: "Adonis"},
          {en: "Nizar Qabbani", es: "Nizar Qabbani", de: "Nizar Qabbani", nl: "Nizar Qabbani"}
        ],
        correct: 0,
        explanation: {
          en: "Badr Shakir al-Sayyab pioneered free verse in Arabic poetry in the 1950s, breaking from traditional forms. His innovative work influenced modern Arabic literature, though he died young at 38 from illness and poverty.",
          es: "Badr Shakir al-Sayyab fue pionero del verso libre en la poesía árabe en la década de 1950, rompiendo con las formas tradicionales. Su trabajo innovador influyó en la literatura árabe moderna, aunque murió joven a los 38 años por enfermedad y pobreza.",
          de: "Badr Shakir al-Sayyab war in den 1950er Jahren Pionier des freien Verses in der arabischen Poesie und brach mit traditionellen Formen. Seine innovative Arbeit beeinflusste die moderne arabische Literatur, obwohl er jung mit 38 Jahren an Krankheit und Armut starb.",
          nl: "Badr Shakir al-Sayyab was een pionier van het vrije vers in de Arabische poëzie in de jaren 1950, waarbij hij brak met traditionele vormen. Zijn innovatieve werk beïnvloedde de moderne Arabische literatuur, hoewel hij jong stierf op 38-jarige leeftijd aan ziekte en armoede."
        }
      },
      {
        question: {
          en: "Which Mexican author wrote 'Pedro Páramo', a pioneering work of magical realism set in a ghost town?",
          es: "¿Qué autor mexicano escribió 'Pedro Páramo', una obra pionera del realismo mágico ambientada en un pueblo fantasma?",
          de: "Welcher mexikanische Autor schrieb 'Pedro Páramo', ein wegweisendes Werk des magischen Realismus in einer Geisterstadt?",
          nl: "Welke Mexicaanse auteur schreef 'Pedro Páramo', een baanbrekend werk van magisch realisme gesitueerd in een spookstad?"
        },
        options: [
          {en: "Carlos Fuentes", es: "Carlos Fuentes", de: "Carlos Fuentes", nl: "Carlos Fuentes"},
          {en: "Juan Rulfo", es: "Juan Rulfo", de: "Juan Rulfo", nl: "Juan Rulfo"},
          {en: "Octavio Paz", es: "Octavio Paz", de: "Octavio Paz", nl: "Octavio Paz"},
          {en: "Elena Garro", es: "Elena Garro", de: "Elena Garro", nl: "Elena Garro"}
        ],
        correct: 1,
        explanation: {
          en: "Juan Rulfo wrote 'Pedro Páramo' (1955), a slim novel that profoundly influenced Latin American literature. The non-linear narrative blurs the line between living and dead, inspiring García Márquez and other magical realists.",
          es: "Juan Rulfo escribió 'Pedro Páramo' (1955), una novela breve que influyó profundamente en la literatura latinoamericana. La narrativa no lineal difumina la línea entre vivos y muertos, inspirando a García Márquez y otros realistas mágicos.",
          de: "Juan Rulfo schrieb 'Pedro Páramo' (1955), einen schmalen Roman, der die lateinamerikanische Literatur tiefgreifend beeinflusste. Die nicht-lineare Erzählung verwischt die Grenze zwischen Lebenden und Toten und inspirierte García Márquez und andere magische Realisten.",
          nl: "Juan Rulfo schreef 'Pedro Páramo' (1955), een dunne roman die de Latijns-Amerikaanse literatuur diepgaand beïnvloedde. Het niet-lineaire verhaal vervaagt de grens tussen levenden en doden, en inspireerde García Márquez en andere magisch-realisten."
        }
      },
      {
        question: {
          en: "Which Vietnamese author wrote 'The Sorrow of War', based on his experiences in the Vietnam War?",
          es: "¿Qué autor vietnamita escribió 'La tristeza de la guerra', basado en sus experiencias en la Guerra de Vietnam?",
          de: "Welcher vietnamesische Autor schrieb 'Die Trauer des Krieges', basierend auf seinen Erfahrungen im Vietnamkrieg?",
          nl: "Welke Vietnamese auteur schreef 'The Sorrow of War', gebaseerd op zijn ervaringen in de Vietnamoorlog?"
        },
        options: [
          {en: "Bao Ninh", es: "Bao Ninh", de: "Bao Ninh", nl: "Bao Ninh"},
          {en: "Duong Thu Huong", es: "Duong Thu Huong", de: "Duong Thu Huong", nl: "Duong Thu Huong"},
          {en: "Nguyen Huy Thiep", es: "Nguyen Huy Thiep", de: "Nguyen Huy Thiep", nl: "Nguyen Huy Thiep"},
          {en: "Le Luu", es: "Le Luu", de: "Le Luu", nl: "Le Luu"}
        ],
        correct: 0,
        explanation: {
          en: "Bao Ninh wrote 'The Sorrow of War' (1991), drawing on his experiences as one of ten survivors from his 500-soldier battalion. The novel offers a Vietnamese perspective on the war, exploring trauma, memory, and loss.",
          es: "Bao Ninh escribió 'La tristeza de la guerra' (1991), basándose en sus experiencias como uno de los diez sobrevivientes de su batallón de 500 soldados. La novela ofrece una perspectiva vietnamita sobre la guerra, explorando trauma, memoria y pérdida.",
          de: "Bao Ninh schrieb 'Die Trauer des Krieges' (1991), basierend auf seinen Erfahrungen als einer von zehn Überlebenden seines 500-Mann-Bataillons. Der Roman bietet eine vietnamesische Perspektive auf den Krieg und erforscht Trauma, Erinnerung und Verlust.",
          nl: "Bao Ninh schreef 'The Sorrow of War' (1991), gebaseerd op zijn ervaringen als een van de tien overlevenden van zijn 500-man bataljon. De roman biedt een Vietnamees perspectief op de oorlog en verkent trauma, geheugen en verlies."
        }
      },
      {
        question: {
          en: "Which Zimbabwean author wrote 'Nervous Conditions', exploring colonialism and gender in Rhodesia?",
          es: "¿Qué autora zimbabuense escribió 'Condiciones nerviosas', explorando el colonialismo y el género en Rhodesia?",
          de: "Welche simbabwische Autorin schrieb 'Nervous Conditions', die Kolonialismus und Geschlecht in Rhodesien erforscht?",
          nl: "Welke Zimbabwaanse auteur schreef 'Nervous Conditions', waarin kolonialisme en gender in Rhodesië worden verkend?"
        },
        options: [
          {en: "Doris Lessing", es: "Doris Lessing", de: "Doris Lessing", nl: "Doris Lessing"},
          {en: "Tsitsi Dangarembga", es: "Tsitsi Dangarembga", de: "Tsitsi Dangarembga", nl: "Tsitsi Dangarembga"},
          {en: "Yvonne Vera", es: "Yvonne Vera", de: "Yvonne Vera", nl: "Yvonne Vera"},
          {en: "NoViolet Bulawayo", es: "NoViolet Bulawayo", de: "NoViolet Bulawayo", nl: "NoViolet Bulawayo"}
        ],
        correct: 1,
        explanation: {
          en: "Tsitsi Dangarembga wrote 'Nervous Conditions' (1988), the first novel published in English by a Black Zimbabwean woman. The semi-autobiographical work examines the intersection of colonialism, patriarchy, and education through a young girl's coming of age.",
          es: "Tsitsi Dangarembga escribió 'Condiciones nerviosas' (1988), la primera novela publicada en inglés por una mujer negra zimbabuense. La obra semi-autobiográfica examina la intersección del colonialismo, patriarcado y educación a través de la mayoría de edad de una joven.",
          de: "Tsitsi Dangarembga schrieb 'Nervous Conditions' (1988), der erste auf Englisch veröffentlichte Roman einer schwarzen simbabwischen Frau. Das semi-autobiografische Werk untersucht die Schnittstelle von Kolonialismus, Patriarchat und Bildung durch das Erwachsenwerden eines jungen Mädchens.",
          nl: "Tsitsi Dangarembga schreef 'Nervous Conditions' (1988), de eerste roman gepubliceerd in het Engels door een zwarte Zimbabwaanse vrouw. Het semi-autobiografische werk onderzoekt het kruispunt van kolonialisme, patriarchaat en onderwijs door het volwassen worden van een jong meisje."
        }
      },
      {
        question: {
          en: "Which Romanian-German author wrote 'The Hunger Angel', about deportation to Soviet labor camps?",
          es: "¿Qué autora rumano-alemana escribió 'El ángel del hambre', sobre la deportación a campos de trabajo soviéticos?",
          de: "Welche rumänisch-deutsche Autorin schrieb 'Atemschaukel', über die Deportation in sowjetische Arbeitslager?",
          nl: "Welke Roemeens-Duitse auteur schreef 'The Hunger Angel', over deportatie naar Sovjet-werkkampen?"
        },
        options: [
          {en: "Herta Müller", es: "Herta Müller", de: "Herta Müller", nl: "Herta Müller"},
          {en: "Elfriede Jelinek", es: "Elfriede Jelinek", de: "Elfriede Jelinek", nl: "Elfriede Jelinek"},
          {en: "Christa Wolf", es: "Christa Wolf", de: "Christa Wolf", nl: "Christa Wolf"},
          {en: "Ingeborg Bachmann", es: "Ingeborg Bachmann", de: "Ingeborg Bachmann", nl: "Ingeborg Bachmann"}
        ],
        correct: 0,
        explanation: {
          en: "Herta Müller wrote 'The Hunger Angel' (2009) based on poet Oskar Pastior's experiences in Soviet labor camps. Müller, who won the Nobel Prize in 2009, writes about life under Romanian dictatorship and ethnic German experiences.",
          es: "Herta Müller escribió 'El ángel del hambre' (2009) basado en las experiencias del poeta Oskar Pastior en campos de trabajo soviéticos. Müller, quien ganó el Nobel en 2009, escribe sobre la vida bajo la dictadura rumana y las experiencias de los alemanes étnicos.",
          de: "Herta Müller schrieb 'Atemschaukel' (2009), basierend auf den Erfahrungen des Dichters Oskar Pastior in sowjetischen Arbeitslagern. Müller, die 2009 den Nobelpreis gewann, schreibt über das Leben unter rumänischer Diktatur und die Erfahrungen ethnischer Deutscher.",
          nl: "Herta Müller schreef 'The Hunger Angel' (2009) gebaseerd op de ervaringen van dichter Oskar Pastior in Sovjet-werkkampen. Müller, die in 2009 de Nobelprijs won, schrijft over het leven onder de Roemeense dictatuur en ervaringen van etnische Duitsers."
        }
      },
      {
        question: {
          en: "Which Sri Lankan author wrote 'The English Patient', later adapted into an Academy Award-winning film?",
          es: "¿Qué autor de Sri Lanka escribió 'El paciente inglés', posteriormente adaptado en una película ganadora del Oscar?",
          de: "Welcher sri-lankische Autor schrieb 'Der englische Patient', das später zu einem Oscar-prämierten Film adaptiert wurde?",
          nl: "Welke Sri Lankaanse auteur schreef 'The English Patient', later bewerkt tot een Oscar-winnende film?"
        },
        options: [
          {en: "Michael Ondaatje", es: "Michael Ondaatje", de: "Michael Ondaatje", nl: "Michael Ondaatje"},
          {en: "Shyam Selvadurai", es: "Shyam Selvadurai", de: "Shyam Selvadurai", nl: "Shyam Selvadurai"},
          {en: "Romesh Gunesekera", es: "Romesh Gunesekera", de: "Romesh Gunesekera", nl: "Romesh Gunesekera"},
          {en: "Jean Arasanayagam", es: "Jean Arasanayagam", de: "Jean Arasanayagam", nl: "Jean Arasanayagam"}
        ],
        correct: 0,
        explanation: {
          en: "Michael Ondaatje wrote 'The English Patient' (1992), which won the Booker Prize. The novel interweaves the stories of four characters in an Italian villa at the end of World War II, exploring themes of identity, nationality, and love.",
          es: "Michael Ondaatje escribió 'El paciente inglés' (1992), que ganó el Premio Booker. La novela entrelaza las historias de cuatro personajes en una villa italiana al final de la Segunda Guerra Mundial, explorando temas de identidad, nacionalidad y amor.",
          de: "Michael Ondaatje schrieb 'Der englische Patient' (1992), der den Booker Prize gewann. Der Roman verwebt die Geschichten von vier Charakteren in einer italienischen Villa am Ende des Zweiten Weltkriegs und erforscht Themen von Identität, Nationalität und Liebe.",
          nl: "Michael Ondaatje schreef 'The English Patient' (1992), dat de Booker Prize won. De roman verweeft de verhalen van vier personages in een Italiaanse villa aan het einde van de Tweede Wereldoorlog, waarbij thema's als identiteit, nationaliteit en liefde worden verkend."
        }
      },
      {
        question: {
          en: "Which Palestinian poet is considered the national poet of Palestine for his resistance poetry?",
          es: "¿Qué poeta palestino es considerado el poeta nacional de Palestina por su poesía de resistencia?",
          de: "Welcher palästinensische Dichter gilt als Nationaldichter Palästinas für seine Widerstandspoesie?",
          nl: "Welke Palestijnse dichter wordt beschouwd als de nationale dichter van Palestina voor zijn verzetspoëzie?"
        },
        options: [
          {en: "Mahmoud Darwish", es: "Mahmoud Darwish", de: "Mahmoud Darwish", nl: "Mahmoud Darwish"},
          {en: "Ghassan Kanafani", es: "Ghassan Kanafani", de: "Ghassan Kanafani", nl: "Ghassan Kanafani"},
          {en: "Fadwa Tuqan", es: "Fadwa Tuqan", de: "Fadwa Tuqan", nl: "Fadwa Tuqan"},
          {en: "Taha Muhammad Ali", es: "Taha Muhammad Ali", de: "Taha Muhammad Ali", nl: "Taha Muhammad Ali"}
        ],
        correct: 0,
        explanation: {
          en: "Mahmoud Darwish's poetry gave voice to the Palestinian experience of exile and occupation. His work combines personal lyricism with political themes, making him one of the Arab world's most celebrated poets until his death in 2008.",
          es: "La poesía de Mahmoud Darwish dio voz a la experiencia palestina de exilio y ocupación. Su obra combina lirismo personal con temas políticos, convirtiéndolo en uno de los poetas más celebrados del mundo árabe hasta su muerte en 2008.",
          de: "Mahmoud Darwishs Poesie gab der palästinensischen Erfahrung von Exil und Besatzung eine Stimme. Seine Arbeit kombiniert persönliche Lyrik mit politischen Themen und machte ihn bis zu seinem Tod 2008 zu einem der gefeiertsten Dichter der arabischen Welt.",
          nl: "Mahmoud Darwish's poëzie gaf een stem aan de Palestijnse ervaring van ballingschap en bezetting. Zijn werk combineert persoonlijk lyrisme met politieke thema's, waardoor hij tot zijn dood in 2008 een van de meest gevierde dichters van de Arabische wereld werd."
        }
      },
      {
        question: {
          en: "Which Algerian author wrote 'The Stranger', exploring existential themes in colonial Algeria?",
          es: "¿Qué autor argelino escribió 'El extranjero', explorando temas existenciales en la Argelia colonial?",
          de: "Welcher algerische Autor schrieb 'Der Fremde', der existentielle Themen im kolonialen Algerien erforscht?",
          nl: "Welke Algerijnse auteur schreef 'De Vreemdeling', waarin existentiële thema's in koloniaal Algerije worden verkend?"
        },
        options: [
          {en: "Albert Camus", es: "Albert Camus", de: "Albert Camus", nl: "Albert Camus"},
          {en: "Assia Djebar", es: "Assia Djebar", de: "Assia Djebar", nl: "Assia Djebar"},
          {en: "Kateb Yacine", es: "Kateb Yacine", de: "Kateb Yacine", nl: "Kateb Yacine"},
          {en: "Mohammed Dib", es: "Mohammed Dib", de: "Mohammed Dib", nl: "Mohammed Dib"}
        ],
        correct: 0,
        explanation: {
          en: "Albert Camus wrote 'The Stranger' (1942), a foundational work of existentialism set in Algeria. The novel's protagonist, Meursault, embodies absurdist philosophy through his emotional detachment. Camus won the Nobel Prize in 1957.",
          es: "Albert Camus escribió 'El extranjero' (1942), una obra fundamental del existencialismo ambientada en Argelia. El protagonista de la novela, Meursault, encarna la filosofía absurdista a través de su desapego emocional. Camus ganó el Nobel en 1957.",
          de: "Albert Camus schrieb 'Der Fremde' (1942), ein grundlegendes Werk des Existentialismus, das in Algerien spielt. Der Protagonist des Romans, Meursault, verkörpert die absurdistische Philosophie durch seine emotionale Distanziertheit. Camus gewann 1957 den Nobelpreis.",
          nl: "Albert Camus schreef 'De Vreemdeling' (1942), een fundamenteel werk van het existentialisme gesitueerd in Algerije. De protagonist van de roman, Meursault, belichaamt de absurdistische filosofie door zijn emotionele afstandelijkheid. Camus won de Nobelprijs in 1957."
        }
      }
    ]
  };

  if (typeof module !== 'undefined' && module.exports) {
    module.exports = level6;
  } else if (typeof window !== 'undefined') {
    window.level6 = level6;
  }
})();
